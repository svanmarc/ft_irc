...
void MessageHandler::MessageWhoisUser(ClientHandler *clientHandler, ClientHandler *targetClient) {
    ...
    response +=
            std::string(targetClient->getUser().getRealname()) + " " + targetClient->getServer()->getServerName() + " ";
...
void MessageHandler::MessageWhois(ClientHandler *clientHandler, ClientHandler *targetClient) {
    ...
    std::string w1 = templagte + "311 " + nickName + " " + nickName + " " + realname + realnameTile + " " + userServer +
                     " * :" + realname;
    std::string w3 =
            templagte + "312 " + nickName + " " + nickName + " " + serverName + ": ubuntu-linux default configuration";
    ...
    std::string w4 = templagte + "317 " + nickName + " " + nickName + " " + idleTimeStr.str() + " " +
                     signonTimeStr.str() + " :seconds idle, signon time";
...
void MessageHandler::MessageWhoisNotFound(ClientHandler *clientHandler, const std::string &targetNickname) {
    MessageHandler::sendResponse(clientHandler, IRCConstants::ERR_NOSUCHNICK,
                                 targetNickname + " UnknownNick :No such nick/channel");
}
...
void MessageHandler::sendPong(ClientHandler *clientHandler) {
    std::string message =
            "PONG " + clientHandler->getServer()->getServerName() + " :" + clientHandler->getServer()->getServerName();
